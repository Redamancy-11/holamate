...
def generate_local_menu(category, name):
    ...
    # Shuffle and select 18-25 items
...
def search_google_maps(query):
    ...
    params = {
        ...
        # Center the search around FPT University Hanoi
    ...
    try:
        ...
        print(f"❌ Error querying SerpAPI for '{query}': {e}")
        return []
...
def main():
    ...
    queries = [
        "cơm gần Đại học FPT Hoà Lạc",
        "bún phở gần Đại học FPT Hoà Lạc",
        "cafe trà sữa gần Đại học FPT Hoà Lạc",
        "quán ăn vặt Tân Xã Thạch Thất",
        "lẩu nướng Tân Xã Thạch Thất",
        "nhà hàng ăn uống gần Đại học FPT Hà Nội",
        "quán cơm bình dân Bình Yên Thạch Thất"
    ]
    ...
    for place in all_scraped_places:
        ...
        # Fallback coordinates within 1.5km of FPT campus
        if lat is None or lng is None:
            lat = 21.0122 + random.uniform(-0.012, 0.012)
            lng = 105.5255 + random.uniform(-0.012, 0.012)
        ...
        # Look if already exists in local_vendors_override.json
        match_idx = -1
        ...
        # Generate fresh, rich local menu
        ...
        if match_idx >= 0:
            # Update only if existing menu is sparse
