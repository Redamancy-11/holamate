...
def main():
    ...
    # Massive permuted search task set to maximize coverage and density
    search_tasks = [
        # General multi-page sweeps
        ("quán ăn Hoà Lạc", [0, 20, 40]),
        ("nhà hàng Hoà Lạc", [0, 20, 40]),
        ("cafe Hoà Lạc", [0, 20, 40]),
        ("trà sữa Hoà Lạc", [0, 20, 40]),
        # Specific item mappings
        ("bún bò huế Hoà Lạc", [0]),
        ("phở bò Hoà Lạc", [0]),
        ("bún đậu Hoà Lạc", [0]),
        ("cơm tấm Hoà Lạc", [0]),
        ("bánh mì Hoà Lạc", [0]),
        ...
        ("quán nhậu Hoà Lạc", [0]),
        ("bánh xèo Hoà Lạc", [0]),
        ("kem dừa Côn Đảo Hoà Lạc", [0]),
        ("sữa chua trân châu Hạ Long Hoà Lạc", [0]),
        ("bún chả Hoà Lạc", [0]),
        ("mì cay Hoà Lạc", [0]),
        ("tạp hóa Hoà Lạc", [0]),
        ("siêu thị mini Hoà Lạc", [0])
    ]
    ...
    print(f"\n==================================================")
    print(f"Total unique places found in massive sweep: {len(all_scraped_places)}")
    print(f"==================================================")
    ...
    for place in all_scraped_places:
        ...
        # Check if bounds are within general Hoa Lac (otherwise fallback centered)
        # Lat range: 20.95 to 21.07, Lng range: 105.45 to 105.60
        is_in_bounds = True
        if lat is not None and (lat < 20.95 or lat > 21.07):
            is_in_bounds = False
        if lng is not None and (lng < 105.45 or lng > 105.60):
            is_in_bounds = False

        if lat is None or lng is None or not is_in_bounds:
            lat = 21.0122 + random.uniform(-0.018, 0.018)
            lng = 105.5255 + random.uniform(-0.018, 0.018)
        ...
        vendor_data = {
            ...
            "phone": phone,
            "tags": tags,
            "tips": f"Quán được đánh giá {rating}★ tại khu vực Hoà Lạc.",
            "menu": generated_menu
        }
